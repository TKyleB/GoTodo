...
const deleteSnippetById = `-- name: DeleteSnippetById :exec
DELETE FROM snippets WHERE snippets.id = $1
`

func (q *Queries) DeleteSnippetById(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.ExecContext(ctx, deleteSnippetById, id)
	return err
}
